...
exports.exitToDashboard = exports.getMyResults = exports.updateSystemCheck = exports.submitAssessment = exports.autosaveCode = exports.startAssessment = exports.getCandidateAssessment = exports.joinAssessment = void 0;
...
const getCandidateAssessment = async (req, res) => {
    try {
        ...
        if (attempt.expiresAt && new Date() > attempt.expiresAt && attempt.status === 'IN_PROGRESS') {
            await db_js_1.prisma.candidateAttempt.update({
                where: { id: attempt.id },
                data: { status: 'EXPIRED' }
            });
...
exports.getCandidateAssessment = getCandidateAssessment;
const startAssessment = async (req, res) => {
    try {
        ...
        for (const q of attempt.assessment.questions) {
            const existingAns = await db_js_1.prisma.candidateAnswer.findUnique({
            ...
            if (!existingAns) {
                ...
                await db_js_1.prisma.candidateAnswer.create({
                    data: {
                        attemptId: attempt.id,
                        questionId: q.id,
                        code: initialCode,
...
const autosaveCode = async (req, res) => {
    try {
        const attemptId = req.user?.attemptId || req.user?.id;
        ...
        const answer = await db_js_1.prisma.candidateAnswer.upsert({
            where: {
...
exports.autosaveCode = autosaveCode;
const submitAssessment = async (req, res) => {
    try {
        const attemptId = req.user?.attemptId || req.user?.id;
        const { answers: submittedAnswers } = req.body || {};
        let attempt = await db_js_1.prisma.candidateAttempt.findUnique({
            where: { id: attemptId },
            include: {
                assessment: {
                    include: {
        ...
        if (submittedAnswers && typeof submittedAnswers === 'object') {
            for (const questionId of Object.keys(submittedAnswers)) {
                ...
                if (item && typeof item === 'object') {
                    await db_js_1.prisma.candidateAnswer.upsert({
                        where: {
                            attemptId_questionId: {
                                attemptId: attempt.id,
                                questionId
            ...
            attempt = (await db_js_1.prisma.candidateAttempt.findUnique({
                where: { id: attemptId },
                include: {
                    assessment: {
                        include: {
        ...
        for (const question of attempt.assessment.questions) {
            ...
            if (code.length > 0) {
                if (question.testCases && question.testCases.length > 0) {
                    ...
                    const execRes = await codeRunner_js_1.CodeRunnerService.execute(language, code, question.testCases);
                    ...
                    if (answer) {
                        await db_js_1.prisma.candidateAnswer.update({
                            where: { id: answer.id },
                            data: {
                                score: questionScore,
                    ...
                    const execRes = await codeRunner_js_1.CodeRunnerService.execute(language, code, [{ input: '', expectedOutput: '', isHidden: false }]);
        ...
        const now = new Date();
        const finalAttempt = await db_js_1.prisma.candidateAttempt.update({
            where: { id: attempt.id },
            data: {
                status: 'SUBMITTED',
                submittedAt: now,
...
const updateSystemCheck = async (req, res) => {
    try {
        const attemptId = req.user?.attemptId || req.user?.id;
...
exports.updateSystemCheck = updateSystemCheck;
const getMyResults = async (req, res) => {
    try {
        ...
        const attempts = await db_js_1.prisma.candidateAttempt.findMany({
...
const exitToDashboard = async (req, res) => {
    try {
        ...
        const account = await db_js_1.prisma.candidateAccount.findUnique({ where: { email: email.toLowerCase() } });
        const token = jsonwebtoken_1.default.sign({
...
exports.exitToDashboard = exitToDashboard;
