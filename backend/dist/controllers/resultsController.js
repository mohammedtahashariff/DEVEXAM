...
exports.getCandidateAttemptResult = exports.getAllCandidates = exports.getDashboardStats = void 0;
const db_js_1 = require("../db.js");
const getDashboardStats = async (req, res) => {
    try {
        const [totalAssessments, activeAssessments, totalCandidates, totalRegistered, completedAttempts, allAttempts, proctoringEvents] = await Promise.all([
            db_js_1.prisma.assessment.count(),
            db_js_1.prisma.assessment.count({ where: { status: 'ACTIVE' } }),
            db_js_1.prisma.candidateAttempt.count(),
            db_js_1.prisma.candidateAccount.count(),
            db_js_1.prisma.candidateAttempt.findMany({
            ...
            db_js_1.prisma.proctoringEvent.count({
        ...
        const avgScore = completedCount > 0
            ...
            : 0;
        const flaggedCount = await db_js_1.prisma.candidateAttempt.count({
            where: {
                OR: [
                    { tabSwitches: { gte: 2 } },
        ...
        completedAttempts.forEach(att => {
            if (att.score <= 20)
                scoreRanges[0].count++;
            else if (att.score <= 40)
                scoreRanges[1].count++;
            else if (att.score <= 60)
                scoreRanges[2].count++;
        ...
        const assessmentsWithAttempts = await db_js_1.prisma.assessment.findMany({
            take: 5,
            orderBy: { createdAt: 'desc' },
            include: {
...
const getAllCandidates = async (req, res) => {
    try {
        ...
        const accounts = await db_js_1.prisma.candidateAccount.findMany({
            where: accountWhere,
            orderBy: { createdAt: 'desc' }
        });
...
const getCandidateAttemptResult = async (req, res) => {
    try {
...
exports.getCandidateAttemptResult = getCandidateAttemptResult;
