...
export const getProctoringLogs = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const [total, events] = await Promise.all([
      prisma.proctoringEvent.count({ where: whereClause }),
...
export const getLiveCandidateMonitoring = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const activeAttempts = await prisma.candidateAttempt.findMany({
      where,
    ...
    const liveData = activeAttempts.map(attempt => {
      ...
      let riskLevel: 'ACTIVE' | 'WARNING' | 'REVIEW_REQUIRED' = 'ACTIVE';
